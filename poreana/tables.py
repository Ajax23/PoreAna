...
def mc_results(link, print_con=True, sections={"pore": [], "res":[], "box": []}):
    ...
    if "pore" in data:
        pore = data["pore"]
        z_length = round(pore["box"][2],2)
        res = round(pore["res"],2)
        ...
        data = [ [box,diff[0], diff[3]],[res,diff_res[0], diff_res[3]], [pore,diff_pore[0],diff_pore[3]]]
        df_mc_results = pd.DataFrame(data, index=list(["Box", "Reservoir","Pore"]), columns=list(['Area','Diffusion (10^-9  m^2s^-1)' ,'Residual (10^-9 m2s^-1)']))
    else:
        box = data["box"]
        z_length = round(box["length"][2],2)
        box = [0,z_length]
        data = [ [box, diff[0], diff[3]]]
        if section["box"]:
            diff_section = diffusion.mc_fit(link,  section = section["box"], is_print=False, is_plot = False)
            data.append([section["box"],diff_section[0],diff_section[3]])
            df_mc_results = pd.DataFrame(data, index=list(["Box","Section"]), columns=list(['Area','Diffusion (10^-9  m^2s^-1)' ,'Residual (10^-9 m2s^-1)']))
        else:
            df_mc_results = pd.DataFrame(data, index=list(["Box"]), columns=list(['Area','Diffusion (10^-9  m^2s^-1)' ,'Residual (10^-9 m2s^-1)']))
