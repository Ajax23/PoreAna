...
def mc_fit(link, len_step=[], is_std=True, is_pore=False, section=[]):
    ...
    With :math:`\\mathrm{is}\_{\\mathrm{pore}}=True` only the pore area can be
    considered.
    With :math:`\\mathrm{section}=[start,end]` can be considered a specific
    area in the box.
    ...
    is_pore : bool, optional
        Is a diffusion data object file of a pore system load in the function
    ...
    if is_pore and not section:
        ...
        index_end = np.digitize((box[2]-res), bins)
    ...
    if section:
        # Cut profile
        # Calculated start and end bin index of the pore area
        index_start = np.digitize(section[0], bins)
        index_end = np.digitize(section[1], bins)

        # Save for all lag times the cutted profile
        for i in len_step:
            diff_bin_vec[i] = {}
            diff_bin_vec[i] = diff_bin[i][index_start:index_end]
